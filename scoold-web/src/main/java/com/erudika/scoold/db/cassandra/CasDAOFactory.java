...
//	public static final CF<String> COUNTERS = new CF<String>("Counters");
	...
	public <T extends PObject> GenericDAO<?> getDAO(Class<T> clazz) {
		...
		if(clazz.equals(User.class)) return new CasUserDAO<User>();
		else if(clazz.equals(Media.class)) return new CasMediaDAO<Media>();
		else if(clazz.equals(Revision.class)) return new CasRevisionDAO<Revision>();
		else if(clazz.equals(School.class)) return new CasSchoolDAO<School>();
		else if(clazz.equals(Classunit.class)) return new CasClassUnitDAO<Classunit>();
		else if(clazz.equals(Group.class)) return new CasGroupDAO<Group>();
		else if(clazz.equals(Message.class)) return new CasMessageDAO<Message>();
		else if(clazz.equals(Comment.class)) return new CasCommentDAO<Comment>();
		else if(clazz.equals(Report.class)) return new CasReportDAO<Report>();
		else if(clazz.equals(Post.class)) return new CasPostDAO<Post>();
		else if(clazz.equals(Tag.class)) return new CasTagDAO<Tag>();
		else return null;
    }
